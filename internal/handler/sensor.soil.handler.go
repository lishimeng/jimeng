...
func SoilSensorHandler(session mq.Session, payload mq.PayloadRx) {

	log.Debug("soil sensor handler: %s", payload.Data)
...
func downControll(session mq.Session, wanted bool) {

	Shadow.ChangeState = wanted
	SyncState(session)
}